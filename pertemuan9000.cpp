...
// Posisi kamera awal: sedikit di luar garasi
float cameraPosX = 0.0f;
float cameraPosY = 2.0f;
float cameraPosZ = 8.0f;  // Diubah dari 10 ke 8 agar lebih dekat
...
void drawHalfWallWithSlidingDoor()
{
    ...
    float wallLength = 6.8f; // Panjang masing-masing bagian dinding (sedikit dikurangi agar pas)
    
    // Bagian kiri dinding (tetap) - agak geser ke kanan sedikit
    glPushMatrix();
    glTranslatef(-6.6f, wallHeight/2, 6.0f); // Geser dari -6.5 ke -6.6
    drawScaledCube(wallLength, wallHeight, wallThickness);
    glPopMatrix();
    
    // Bagian kanan dinding (bagian yang bergerak sebagai pintu) - agak geser ke kiri sedikit
    glPushMatrix();
    // doorOffset: 0 = tertutup (posisi semula), -6.6 = terbuka (geser ke kiri)
    glTranslatef(6.6f + doorOffset, wallHeight/2, 6.0f); // Geser dari 6.5 ke 6.6
    drawScaledCube(wallLength, wallHeight, wallThickness);
    glPopMatrix();
}
...
void updateDoor()
{
    if (doorOpen && doorOffset > -6.6f) {
        // Pintu terbuka: geser ke kiri
        doorOffset -= doorSpeed;
        if (doorOffset < -6.6f) doorOffset = -6.6f;
